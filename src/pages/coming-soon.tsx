...
export default function ComingSoonPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Filter only coming-soon stage products
      const comingSoonProducts = allProducts.filter((p: Product) => 
        (p.stage || 'voting') === 'coming-soon'
      );
        ...
        <div className="relative overflow-hidden pt-8 pb-16">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start justify-end mb-8">
              {/* Right: Friday Countdown */}
              <div className="flex items-center gap-3 bg-green-900/30 border border-green-500/30 rounded-xl px-4 py-3 backdrop-blur-sm">
                <div className="w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center">
                  <Calendar className="w-5 h-5 text-green-400" />
                </div>
                <div>
                  <div className="text-sm font-semibold text-green-300">Products Go Live This Friday</div>
                  <div className="flex items-center gap-2">
                    {(() => {
                      const daysToFriday = getDaysUntilNextFriday();
                      const waitingProducts = getProductsWaitingForFriday(
                        products as any, 
                        DEFAULT_LIFECYCLE_CONFIG
                      );
